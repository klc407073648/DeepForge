...
import { BACKEND_NOT_READY_MSG, isBackendReady, waitForKnowledgeReady } from './helpers'
...
test.describe('RAG 完整流程（需后端就绪）', () => {
  ...
  test('上传文档建索引', async ({ page }) => {
    ...
    await waitForKnowledgeReady(page)
  ...
  test('智能问答返回回答与引用', async ({ page }) => {
    ...
    await expect(page.getByText(uploadedName).first()).toBeVisible()
  })
